...
contract DecubateWhitelisted is Context, Ownable {
   ...
   *
   */
  function whiteList(address addr, bool value) external onlyOwner returns (bool) {
   ...
   *
   */
  function blackList(address addr, bool value) external onlyWhiteListed returns (bool) {
   ...
   *
   */
  function setBlockSellUntil(uint256 value) external onlyWhiteListed returns (bool) {
   ...
   *
   */
  function setPairAddress(address addr) internal returns (bool) {
    pairAddress = addr;
    return true;
  }
}
